...
import argparse
from data_converter import convert_wiki_dataset, convert_cnn_dataset, convert_c4_dataset_eval, convert_openwebtext_dataset
import argparse
...
def simulation_fast(target_model : GraphInferenceEngine, draft_model: GraphInferenceEngine, dataloader: DataLoader, T=0.6, top_p=0.9,
            max_length=512, residual_graph=None, grow_map=None, sampling_callables = None,
            sample_gather_indices = None, tokenizer:AutoTokenizer = None, dtype=torch.float16):
...
def simulation_baseline(target_model : GraphInferenceEngine, dataloader: DataLoader, T=0.6, top_p=0.9, max_length=256, tokenizer = None, dtype=torch.float16):
    num_eval_steps = len(dataloader)
    num_decoding_steps = 0
    total_time = 0.0
    with torch.no_grad():
            ...
            while inner_decoding_step < 32 and terminate == False:
                if inner_decoding_step == 0:
                    ...
                    logits = target_model.inference(input_ids = input_ids, storage_ids=storage_ids[:start_length],
                                                    ...
                                                    attn_mask=attn_mask[:start_length][None, None, :, :])[0][-1]
                    ...
                    logits = target_model.inference(input_ids = input_ids, storage_ids=storage_ids[start_length + inner_decoding_step-1 : start_length + inner_decoding_step],
                                                    ...
                                                    attn_mask=attn_mask[start_length + inner_decoding_step-1 : start_length + inner_decoding_step][None, None, :, :])[0][-1]
                ...
                if (input_ids[0][-1] == 0) or (input_ids[0][-1] == 2): 
                    terminate = True
            torch.cuda.synchronize()
            t2 = time.time()
            total_time += (t2 - t1)
...
def simulation_benchmark(target_model : GraphInferenceEngineTG, draft_model: GraphInferenceEngine, dataloader: DataLoader, T=0.6, top_p=0.9, 
                max_length=512, residual_graph=None, grow_map=None, sampling_callables = None,
                sample_gather_indices = None, tokenizer = None, dtype=torch.float16):
    num_eval_steps = len(dataloader)
    num_decoding_steps = 0
    num_large_model_steps = 0
    initialize_time = 0.0
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(dataloader), total=num_eval_steps):
            ...
            while input_ids.shape[1] < 256 and terminate == False:
                ...
                if (input_ids[0][-1] == 0) or (input_ids[0][-1] == 2) or input_ids.shape[1] >= 256: 
                    terminate = True
                if not terminate:
                    sample_time += a
...
if args.dataset == 'openwebtext':
    tokenized_dataset_eval = convert_openwebtext_dataset(tokenizer=tokenizer).select(eval_list[args.start :args.end])
...
if args.Mode == 'baseline':
    if args.offloading:
        target_model = OffloadEngine(max_length=args.M, model_name_or_path = args.target, dtype = dtype, device="cuda:0")
    else:
        target_model =  GraphInferenceEngineTG(max_length=args.M, model_name_or_path = args.target, dtype = dtype, device="cuda:0")
else:
    draft_model = GraphInferenceEngine(max_length=args.M, model_name_or_path = args.model, dtype = dtype, device="cuda:0")
    if args.offloading:
        target_model = OffloadEngine(max_length=args.M, model_name_or_path = args.target, dtype = dtype, device="cuda:0")
    else:
        target_model =  GraphInferenceEngineTG(max_length=args.M, model_name_or_path = args.target, dtype = dtype, device="cuda:0")
    ...
    for i in range(draft_step - 1):
        ...
        sampling_callables[i] = cuda_graph_for_sampling_without_replacement(
            ...
            temperature=args.T, tree_size=tree_size,dim=args.vocab, dtype=dtype) 
    for i in range(draft_step - 1):
        ith_gather_list = []
...
if args.Mode == 'benchmark':
    simulation_benchmark(target_model=target_model, draft_model=draft_model, dataloader=dataloader, T=args.T, top_p=args.P, 
                                               max_length=args.M, residual_graph = residual_graph, grow_map = grow_map, sampling_callables=sampling_callables, sample_gather_indices = sample_gather_indices, tokenizer=tokenizer, dtype=dtype)
elif args.Mode == 'baseline':
    simulation_baseline(target_model=target_model, dataloader=dataloader, T=args.T, top_p=args.P, max_length=args.M, tokenizer=tokenizer,dtype=dtype)
